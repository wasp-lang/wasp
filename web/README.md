...
The build also generates markdown versions of pages and `llms*.txt` files (see [src/plugins/llm-files](src/plugins/llm-files/)). To catch unintended changes in that output, we keep snapshots of a few representative files in [markdown-snapshots/](markdown-snapshots/) and compare them against the build output in CI on every PR that touches `web/`.
