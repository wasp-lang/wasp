...
The build also generates markdown versions of pages (see [src/plugins/llm-files](src/plugins/llm-files/)) and `llms*.txt` files (see [scripts/generate-llm-files.ts](scripts/generate-llm-files.ts)). To catch unintended changes in that output, we keep snapshots of a few representative files in [markdown-snapshots/](markdown-snapshots/) and compare them against the build output in CI.
