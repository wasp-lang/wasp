...
// Footer appended to each standalone `.md` so an agent that fetched a single
// page can still find the docs index. Every docs page points at its own
// version's doc map (the latest version's map for unprefixed `/docs/...`
// pages); blog and resources, which have no version map, point at llms.txt.
...
// A doc served under "/docs/<version>/..." belongs to that version; the latest
// version (served at "/docs/...") and non-doc pages have no version prefix.
