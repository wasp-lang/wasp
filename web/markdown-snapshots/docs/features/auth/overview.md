...
### [Email »](https://wasp.sh/docs/features/auth/email/overview)

[Email verification, password reset, etc.](https://wasp.sh/docs/features/auth/email/overview)

### [Username & Password »](https://wasp.sh/docs/features/auth/username-and-pass/overview)

[The simplest way to get started](https://wasp.sh/docs/features/auth/username-and-pass/overview)

### [Google »](https://wasp.sh/docs/features/auth/social-auth/google)

[Users sign in with their Google account](https://wasp.sh/docs/features/auth/social-auth/google)

### [Github »](https://wasp.sh/docs/features/auth/social-auth/github)

[Users sign in with their Github account](https://wasp.sh/docs/features/auth/social-auth/github)

### [Keycloak »](https://wasp.sh/docs/features/auth/social-auth/keycloak)

[Users sign in with their Keycloak account](https://wasp.sh/docs/features/auth/social-auth/keycloak)

### [Slack »](https://wasp.sh/docs/features/auth/social-auth/slack)

[Users sign in with their Slack account](https://wasp.sh/docs/features/auth/social-auth/slack)

### [Discord »](https://wasp.sh/docs/features/auth/social-auth/discord)

[Users sign in with their Discord account](https://wasp.sh/docs/features/auth/social-auth/discord)

Click on each auth method for more details.

Let's say we enabled the [Username & password](https://wasp.sh/docs/features/auth/username-and-pass/overview) authentication.

We get an auth backend with signup and login endpoints. We also get the `user` object in our [Operations](https://wasp.sh/docs/features/data/operations/overview) and we can decide what to do based on whether the user is logged in or not.

We would also get the [Auth UI](https://wasp.sh/docs/features/auth/ui) generated for us. We can set up our login and signup pages where our users can **create their account** and **login**. We can then protect certain pages by setting `authRequired: true` for them. This will make sure that only logged-in users can access them.
...
### [Username and password »](https://wasp.sh/docs/features/auth/username-and-pass/overview)

### [Social Auth »](https://wasp.sh/docs/features/auth/social-auth/overview)
...
### [Email »](https://wasp.sh/docs/features/auth/email/create-your-own-ui)

### [Username and password »](https://wasp.sh/docs/features/auth/username-and-pass/create-your-own-ui)

### [Social Auth »](https://wasp.sh/docs/features/auth/social-auth/create-your-own-ui)
...
The previously discussed options should cover the vast majority of cases. But, for the few instances where it is not enough, you can [create your own signup flows](https://wasp.sh/docs/features/auth/advanced/custom-auth-actions), with completely custom logic. This is not recommended, and reserved for advanced use cases. Please check first if other Wasp features (mainly [auth hooks](https://wasp.sh/docs/features/auth/hooks)) can handle your requirements.
...
You can read more about how the `User` is connected to the rest of the auth system and how you can access the user data in the [Accessing user data](https://wasp.sh/docs/features/auth/entities) section of the docs.
...
When using WebSockets, the `user` object is also available on the `socket.data` object. Read more in the [WebSockets section](https://wasp.sh/docs/features/web-sockets#websocketfn).
...
When you are using the default authentication flow, Wasp validates the fields with some default validations. These validations run if you use Wasp's built-in [Auth UI](https://wasp.sh/docs/features/auth/ui) or if you use the provided auth actions.

If you decide to create your [custom auth actions](https://wasp.sh/docs/features/auth/advanced/custom-auth-actions), you'll need to run the validations yourself.

Default validations depend on the auth method you use.

#### Username & Password

If you use [Username & password](https://wasp.sh/docs/features/auth/username-and-pass/overview) authentication, the default validations are:
...
If you use [Email](https://wasp.sh/docs/features/auth/email/overview) authentication, the default validations are:
...
Since Wasp's definition of valid email addresses is wider than the HTML5 grammar (we also support Unicode!), you won't want to use the usual `type="email"` attribute on the `input` element (which follows HTML5 grammar and doesn't accept Unicode). Instead use `type="text"` with `inputMode="email"` and `autoComplete="email"`.
...
- you need to customize the `SignupForm` (in the case of [Email](https://wasp.sh/docs/features/auth/email/overview) or [Username & Password](https://wasp.sh/docs/features/auth/username-and-pass/overview) auth)
...
For example, if you are using [Username & Password](https://wasp.sh/docs/features/auth/username-and-pass/overview), you would add the `auth.methods.usernameAndPassword.userSignupFields` field:
...
Next, let's see how to customize [Auth UI](https://wasp.sh/docs/features/auth/ui) to include those fields.
...
- [Email auth](https://wasp.sh/docs/features/auth/email/create-your-own-ui)
- [Username & password auth](https://wasp.sh/docs/features/auth/username-and-pass/create-your-own-ui)
:::
