...
Thankfully, Waspleau can leverage a powerful and flexible [hook supplied by Wasp](https://wasp.sh/docs/advanced/server-customization/server-config#setupfn-extimport) called `server.setupFn`. This declares a JavaScript function that will be executed on server start. Yahoo! This means we can do things like the following:
