---
title: "Flue Feels Like React, But for Agents"
authors: [vinny]
...
description: "I integrated a Flue agent into a SaaS app and it felt like writing React for agents: hooks, re-renders, and state-driven tools. Here's how Flue made locking an agent to the logged-in user and guarding destructive actions genuinely simple."
---

I just integrated a [Flue](https://flueframework.com) agent into a [SaaS app](https://opensaas.sh). On the surface, it looks like your typical AI chatbot: it sits in a sidebar, you type at it, it types back.
...
But this agent can also *do* things. It can manage the logged-in user's task list and generates a daily schedule from it. Which is really cool, because I can throw a multi-step request at it, like "add a new task called *present Flue in a video*, check off *integrate a Flue agent into Open SaaS*, and generate a schedule for today", and it works through the tools one by one, ticks off each item, and the task list UI on the left updates live as it goes.

![The agent updating the task list UI live](/img/flue-open-saas/agent-updates-ui.webp)

{/* truncate */}

While this is just a simple demo, whenever you give an agent access to user data, you have to be really careful that it's secure. Making sure the agent can only ever touch *this* user's data, and can't nuke anything without explicit confirmation, turned out to be really easy with Flue.

And it's one of the reasons why I really enjoyed working with it in comparison to other agent frameworks I've used.

If you don't know what Flue is: it's a TypeScript framework for building AI agents, from the folks over at [Astro](https://astro.build). There are a lot of agent frameworks out there right now, but Flue has an angle that genuinely separates it from the pack.

Writing Flue feels like writing [React](https://react.dev/), but for agents. And after building a real integration with it, I think that mental model (plus two other things) makes it a really interesting approach. Let's go through all three:
