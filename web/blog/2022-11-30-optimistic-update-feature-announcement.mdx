...
Continue reading to find out what optimistic updates are and how Wasp implements them.
...
Those are all the changes we need, the rest of the code (i.e., `main.wasp`, `queries.js` and `actions.js`) remains the same. We won't describe the API in detail, but if you're curious, everything is covered by [our official docs](/docs/features/data/operations/actions#the-useaction-hook-and-optimistic-updates).
