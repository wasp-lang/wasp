...
> *An AI-assisted coding workflow without "full-stack vision". 🤖*
...
> *Logging in and adding a task with browser automation. 🤖*
...
> *Adding a full-stack "subtasks" feature with our "full-stack vision" workflow. 🤖*
...
As an LLM’s context window fills up, it’s performance degrades. Some devs even suggest starting a new session once the context is 75% full to avoid this, which is possible with the `/clear` command in Claude Code. You can also run the `/compact` command which creates a summary of your current session’s context and passes it along to the next session.
...
> *Some llms.txt provide lots of information up front, while others are simply maps to documentation.
...
- **Running Scripts** — Skills can be useful if you have deterministic tasks you run often, like a deployment script. Define them in a Skill and link those scripts to them, and Claude Code will run them when it deems it fit for the task at hand.
