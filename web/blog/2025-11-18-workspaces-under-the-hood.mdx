...
As part of our push for Wasp 1.0, we're working on a project internally called [Wasp Citizen](https://github.com/wasp-lang/wasp/issues/3119). We're tweaking the way we generate Wasp apps, and we realized that workspaces solve many of our headaches. The next release of Wasp (out soon!) will use workspaces under the hood.

But don't fret! **Your Wasp app will still be a single package project, and you don't need to learn anything new**, everything will work as before. Internally, the generated code will use workspaces to make development more robust and avoid dependency mismatch errors. If that piques your curiosity, the rest of the post will give you some background on how it works.

## A brief history of workspaces
...
This is a trick question. But not in a bad way! In general terms:

**Workspaces are a way to organize multiple JS packages in a single repository, each acting as their own encapsulated unit, but able to easily depend on one another.**

Seems simple enough, right? So why is it a trick question? Because the exact definition of what a workspace is, and the ways in which it can depend on another, depends on the package manager you're using. There's no single unified "workspaces spec", so each package manager has its own take on it, with different configuration formats, commands, and even different ways to declare dependencies.

However, the core functionality that you can expect from workspaces is mostly the same across package managers[^pkg-manager-specifics]. You declare a root folder that contains multiple sub-packages. Each of these packages is called a _workspace_, and the whole set of workspaces is a _project_[^yarn-glossary].

[^pkg-manager-specifics]: The specifics of how to configure each of these features, and any extra features, will depend on the package manager you're using. For example, Yarn requires you to specifically declare workspace dependencies while npm automatically links all of them. Throughout this post, **we'll be using the npm behavior as reference**, since it's the most widely used package manager. But the general concepts apply to any of them.
...
From then on, three main features are expected from workspaces:
...
"right-pad@^1.0.0":
  version: 1.4.0
  resolved: https://registry.npmjs.com/left-pad/-/left-pad-1.4.0.tgz
  dependencies:
    core-pad: ^1.4.0
...
Some scenarios where I'd discourage using workspaces are:
