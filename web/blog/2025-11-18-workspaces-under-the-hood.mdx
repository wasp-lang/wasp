...
The first feature doesn't require much explanation, as it's just the natural way of working with multiple packages. Let's look into the other two, why are they useful, and how they're implemented:
...
Package managers take a shortcut: when looking at how to resolve dependencies, they just **consider all the workspaces as if they were dependencies of a single big package**. This way, the existing dependency resolution algorithm can be reused without changes.
