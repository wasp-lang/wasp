...
And since `emailSender` is what Wasp uses under the hood for everything email-related, this also covers your [email authentication](/docs/features/auth/email/overview) flows - verification emails and password resets now go through Resend too, with zero extra code.
...
Then set `provider: "Resend"`, add your API key, and send your first email. Check out the [email sending docs](/docs/features/email#resend) for the full reference.
