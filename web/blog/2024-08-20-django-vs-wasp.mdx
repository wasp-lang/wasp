...
And that's all it takes to implement full-stack [Auth with Wasp](https://wasp.sh/docs/features/auth/overview)! But that's just one example, you can also add other auth methods easily, like `google: {}`, `gitHub: {}` and `discord: {}` social auth, after configuring the apps and adding your environment variables.
