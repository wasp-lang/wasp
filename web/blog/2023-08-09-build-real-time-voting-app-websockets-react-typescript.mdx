...
Why? Well, besides less configuration, another nice benefit of working with [TypeScript with Wasp](/docs/features/web-sockets), is that you just have to define payload types with matching event names on the server, and those types will get exposed automatically on the client!
