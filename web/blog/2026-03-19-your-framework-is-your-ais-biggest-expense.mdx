---
title: "Your Framework Is Your AI's Biggest Expense"
authors: [vinny]
image: /img/framework-ai-expense/banner.webp
...
[Wasp](https://wasp.sh) is a full-stack framework for React, Node.js, and Prisma with batteries-included. Think Laravel-like productivity for the JS ecosystem but where authentication, routing, operations, database, and cron jobs are defined declaratively as config. 
...
<CodeSnippetSideBySide.Panel title="main.wasp" tabLabel="Wasp">
...
**Cache creation cost.** Next.js had 2.2x more cache creation tokens (343K vs 155K), at $6.25/M that's $2.15 vs $0.97 — a $1.18 difference that accounts for most of the gap. The bigger codebase means more new content being loaded into cache.

**Cache read cost.** Each API turn re-reads the growing context. The Next.js codebase is bigger so each read costs more — $1.14 vs $0.67. Output tokens were nearly identical (~5,400), meaning the AI wrote roughly the same amount of code but had to read far more to do it.
...
<ImgWithCaption alt="Wasp vs Next.js token efficiency" source="img/framework-token-comparison/saas-wasp.webp" caption="We compared the two frameworks on the same feature prompt in the same app: Vercel's SaaS Starter." justifyContent="flex-start" margin="0 0 1rem 0" width="650px" />

We made sure to use the same models (Opus for planning and implementation, Haiku for exploring), same framework-agnostic prompt, and same plan-then-implement flow, outlined in the [test protocol](https://github.com/vincanger/token-compare-nextjs-wasp/blob/main/code-generation-test.md). Measurement scripts were used to pull metrics from Claude Code's detailed JSONL session transcripts.
...
or check us out on [GitHub](https://github.com/wasp-lang/wasp).
