...
Because we're producing normal `route` and `page` spec objects, the rest of Wasp neither knows nor cares that they were generated. So all the features you'd expect keep working, including [type-safe links](/docs/features/pages-and-routes#navigating-between-pages):
...
Wasp routing also supports [dynamic](/docs/features/pages-and-routes#dynamic-segments) (`:id`) and [optional](/docs/features/pages-and-routes#optional-segments) (`:id?`) segments. Written out in the spec by hand, they look like this:
...
export default app({
  spec: [
    ...
    route("RootRoute", "/", page(RootPage), { prerender: true }),
    route("AboutUsRoute", "/about-us", page(AboutUsPage), { prerender: true }),
    route("ProductIdRoute", "/products/:productId", page(ProductIdPage)),
