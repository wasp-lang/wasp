...
Generating two such functions ensures a uniform calling interface across the entire app (both backend and frontend).
...
Wasp's `useQuery` hook takes three arguments:
...
For example, let's assume that Action `A1` and Query `Q1` both use Entity `E1`. If `A1` is executed, `Q1`'s cached result may no longer be up-to-date.
Wasp will therefore invalidate it, making `Q1` up to date again.
