...
- `logout` function
- `useAuth()` React hook
It should be noted that the Signup form automatically logs in the user after signup.
...
### `signup()`
An action for signing up the user. This action does not login a user, you still need to call `login()`.
```js
signup(userFields)
```
#### `userFields: object`
