...
Don't write the Wasp Spec API reference by hand. Wasp autogenerates it into `web/docs/api` from the JSDoc comments in the spec package (`waspc/packages/spec`), so link to the relevant autogenerated page instead (e.g., with a `CardLink`, like the existing feature pages do). If you need to document something the autogenerated reference doesn't cover, add it to the JSDoc comments in the spec package, and it will show up in the autogenerated docs.
