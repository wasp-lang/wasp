...
### Project Commands
- `wasp start` launches the Wasp app in development mode. It automatically opens a browser tab with your application running and watches for any changes to .wasp or files in `src/` to automatically reflect in the browser. It also shows messages from the web app, the server and the database on stdout/stderr. By default, the client runs on port 3000 and the server on 3001, and if those are taken Wasp picks the next free ones. Your app's URL is the client's (`http://localhost:3000`): the client dev server proxies Wasp's routes to the server, so the browser never talks to port 3001 directly. In [split mode](../deployment/intro.md#deployment-modes) there is no proxy and the browser talks to both. Wasp prints the app URL and the internal server URL when it starts your app. Use `--client-port <port>` and `--server-port <port>` to choose the ports yourself.
...
- `wasp build` generates the complete web app code, which is ready for [deployment](../deployment/intro.md). It needs your dependencies installed (`wasp install`). In the default single deployment mode it also builds the client into static files in `.wasp/out/web-app/build`, so give it any client env vars (`REACT_APP_*`) your app uses; in [split mode](../deployment/intro.md#deployment-modes) you build the client yourself. Use this command when you're deploying or ejecting. The generated code is stored in the `.wasp/out` folder.

- `wasp build start` takes the output of `wasp build` and runs it locally the same way it runs in production: one Docker container with the server serving the client, plus a static server for the client in split mode. You can use it to test the production build of your app locally. It accepts `--server-env` and `--client-env` options to specify the environment variables for the server and client, respectively, and the `--server-port` and `--client-port` options to pick the ports (`--client-port` is ignored in single deployment mode, since the server serves the client). This is useful for testing how your app behaves in production, and to check which environment variables are required for the production build to work correctly. For comprehensive documentation and examples, see [Production Build Preview](../deployment/local-testing.md).
