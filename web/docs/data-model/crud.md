...
For the login and signup pages, copy the drop-in Auth UI from the [Auth UI docs](../auth/ui#drop-in-custom-auth-ui). The copied UI imports hooks and generated auth metadata from `wasp/client/auth`, so it stays in sync with your enabled auth methods.
