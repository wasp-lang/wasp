...
You declare queries in the Wasp Spec file and implement them using NodeJS. Wasp not only runs these queries within the server's context but also creates code that enables you to call them from any part of your codebase, whether it's on the client or server side.
...
import { getAllTasks, getFilteredTasks } from './src/queries' with { type: "ref" }
...
It's a good idea to start with the high-level concept (the Query declaration in the Wasp Spec file) and only then deal with the implementation details (the Query's implementation in JavaScript).
:::
...
<Tabs groupId="js-ts">
  ...
  <TabItem value="ts" label="TypeScript">
    ...
    Wasp automatically generates the types `GetAllTasks` and `GetFilteredTasks` based on your Wasp Spec file's declarations:
...
- `queryFn` <Required />
  ...
  The client-side query function generated by Wasp based on a `query` declaration in your Wasp Spec file.
