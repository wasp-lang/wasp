...
Env vars like `WASP_WEB_CLIENT_URL`, `WASP_SERVER_URL` and `PORT` are **required in production** and must be explicitly set. You don't set them in development: Wasp picks the ports your app runs on and derives these values from them, and setting them yourself makes `wasp start` fail.
