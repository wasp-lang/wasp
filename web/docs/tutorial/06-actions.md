...
## !!steps Implementing the Action

Let's now define a TypeScript function for our `createTask` Action.
...
:::tip
We put the function in a new file `src/actions.ts`, but we could have put it anywhere we wanted! There are no limitations here, as long as the reference import in `main.wasp.ts` points to it and the file is located within the `src` directory.
:::

<TutorialAction id="action-create-task-impl" action="APPLY_PATCH" />
...
## !!steps Adding the Form to the Page
