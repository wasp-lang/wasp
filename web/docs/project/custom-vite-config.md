...
| `server.port` | dynamically assigned | Wasp tells the server where to find the client, so it has to know the client's port. Use [`wasp start --client-port`](#custom-dev-server-port) instead. |
