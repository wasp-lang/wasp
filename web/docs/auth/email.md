...
Copy the drop-in login, signup, password reset, and e-mail verification page code from the [Auth UI docs](../auth/ui#drop-in-custom-auth-ui). The copied UI uses generated metadata from `wasp/client/auth`, so adding or removing OAuth providers in `main.wasp.ts` updates the rendered provider buttons after Wasp regenerates the SDK.
