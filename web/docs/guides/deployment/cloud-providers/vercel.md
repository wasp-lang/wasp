...
Wasp expects the connection string in the `DATABASE_URL` environment variable, so we need to copy it over. Use the value of `POSTGRES_URL_NON_POOLING`: this is Supabase's session-mode pooler, which works both for running Prisma migrations and for serving queries. You can use the CLI to copy the value from one environment variable to the other:

```shell
npx vercel env run -- printenv POSTGRES_URL_NON_POOLING
...
1. Create a `vercel.json` that tells Vercel to treat the built server as an Express app:
   ...
   ```json title="vercel.json"
   {
     ...
     "buildCommand": "cd .wasp/out/server && npm install && npx prisma generate --schema=../db/schema.prisma && npm run bundle && npm run db-migrate-prod",
     "outputDirectory": ".wasp/out/server/bundle"
   }
   ```

   The build command mirrors what the generated `Dockerfile` does: it installs the server's dependencies, generates the Prisma client, bundles the server and runs any pending database migrations. Vercel then looks for the bundled `server.js` in the output directory and wraps it in a function.
