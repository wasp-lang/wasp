...
With a single spec, you can tell Wasp to automatically generate server-side logic (i.e., Queries and Actions) for creating, reading, updating and deleting [Entities](./entities). As you update definitions for your Entities, Wasp automatically regenerates the backend logic.
...
Let's create a full-app example that uses automatic CRUD. We'll stick to using the `Task` entity from the previous example, but we'll add a `User` entity and enable [username and password](../auth/username-and-pass/overview) based auth.
...
<Tabs groupId="js-ts">
  ...
  <TabItem value="ts" label="TypeScript">
    ...
    The `Tasks.CreateAction` type works exactly like the types Wasp generates for [Queries](./operations/queries#type-support-for-queries) and [Actions](./operations/actions#type-support-for-actions).
...
For a usage example, check the [example guide](./crud#adding-crud-to-the-task-entity-).
...
All CRUD operations are implemented with [Queries and Actions](./operations/overview) under the hood, which means they come with all the features you'd expect (e.g., automatic SuperJSON serialization, full-stack type safety when using TypeScript)
