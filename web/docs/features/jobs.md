...
3. After successfully defining the job, you can submit work to be done in your [Operations](./data/operations/overview) or [setupFn](../advanced/server-customization/server-config#setup-function) (or any other NodeJS code):
...
In this example, you _don't_ need to invoke anything in <ShowForJs>JavaScript</ShowForJs><ShowForTs>TypeScript</ShowForTs>. You can imagine `mySpecialJob({ name: "Johnny" })` getting automatically scheduled and invoked for you every hour.
...
`PgBoss` requires that your database provider is set to `"postgresql"` in your `schema.prisma` file. Read more about setting the provider [here](./data/databases.md#postgresql).
...
You can read more about escaping JSON in environment variables in the [JSON Env Vars documentation](../advanced/env-vars.md#json-env-vars).
