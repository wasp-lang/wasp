...
Let's say we enabled the [Username & password](./username-and-pass/overview) authentication.

We get an auth backend with signup and login endpoints. We also get the `user` object in our [Operations](../data/operations/overview) and we can decide what to do based on whether the user is logged in or not.

We would also get the [Auth UI](./ui) generated for us. We can set up our login and signup pages where our users can **create their account** and **login**. We can then protect certain pages by setting `authRequired: true` for them. This will make sure that only logged-in users can access them.
...
<LinkGrid
  links={[
    { title: 'Email', linkTo: './email/overview' },
    { title: 'Username and password', linkTo: './username-and-pass/overview' },
...
Please check first if other Wasp features (mainly [auth hooks](./hooks.md)) can handle your requirements.
...
When using WebSockets, the `user` object is also available on the `socket.data` object. Read more in the [WebSockets section](../web-sockets#websocketfn).
...
If you use [Username & password](./username-and-pass/overview) authentication, the default validations are:
...
If you use [Email](./email/overview.md) authentication, the default validations are:
...
you won't want to use the usual `type="email"` attribute on the `input` element (which follows HTML5 grammar and doesn't accept Unicode).
...
- you need to customize the `SignupForm` (in the case of [Email](./email/overview.md) or [Username & Password](./username-and-pass/overview.md) auth)
...
For example, if you are using [Username & Password](./username-and-pass/overview), you would add the `auth.methods.usernameAndPassword.userSignupFields` field:
...
Next, let's see how to customize [Auth UI](./ui) to include those fields.
