...
That is why we believe Wasp needs to be a spec-driven framework and not just a library - we want to capture all parts of the web app into one integrated system that is perfectly tailored just for that purpose.\
...
- **Declarative, statically typed spec** with simple basic rules and **that understands a lot of web app concepts** - "horizontal language". Supports multiple files/modules, libraries.
...
- Besides Wasp Spec, there will also be a **visual builder that generates/edits Wasp code**, allowing non-developers to participate in development. Since Wasp is declarative, we imagine such builder to naturally follow from the Wasp Spec.
...
- While it comes with the official implementation(s), **the Wasp Spec will not be coupled with the single implementation**.
