...
We'll take a look at our Coolify deployment example in the [deployment](/docs/0.21/deployment/deployment-methods/self-hosted#coolify) section. We are using Github Actions to build the Docker images and their Github Container Registry (GHCR) to store them.
...
Check out our instructions for deploying the client app to [Netlify](/docs/0.21/deployment/deployment-methods/paas#netlify) or [Cloudflare](/docs/0.21/deployment/deployment-methods/paas#cloudflare) where you can check out the example deployment using Github Actions.
