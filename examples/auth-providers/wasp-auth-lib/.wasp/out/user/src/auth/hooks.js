...
export const onBeforeSignup = async ({ providerId }) => {
    ...
    console.log(`[hooks] onBeforeSignup: ${providerId.providerName}/${providerId.providerUserId}`);
};
export const onAfterLogin = async ({ providerId, user }) => {
    console.log(`[hooks] onAfterLogin: ${providerId.providerName}/${providerId.providerUserId} -> user ${user.id}`);
};
