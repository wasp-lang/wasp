...
import { findAuthWithUserBy, validateAndGetUserFields } from './utils.js';
 ...
 * entirely.
 */
export async function exchangeRequestForSession(providerId, req) {
 ...
 */
async function resolveExternalSubject(providerId, subjectId, claims, identity, 
...
namespace = providerId, req) {
    ...
    let created;
    try {
        created = await identities.createIdentity(subjectId, {
 ...
 */
export async function provisionAuthUser(providerId, subjectId, claims, identity, namespace) {
 ...
 */
export async function computeProviderUserFields(providerId, claims) {
...
// `invalidateSession`).
export async function invalidateAllSessionsForAuthId(authId) {
