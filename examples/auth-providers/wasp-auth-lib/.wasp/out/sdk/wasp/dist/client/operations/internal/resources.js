...
export function registerActionInProgress(optimisticUpdateTuples) {
    optimisticUpdateTuples.forEach(({ queryKey, updateQuery }) => updateHandlers.add(queryKey, updateQuery));
}
...
async function invalidateQueriesUsing(resources) {
    ...
    await Promise.all(queryCacheKeysToInvalidate.map(queryCacheKey => queryClient.invalidateQueries(queryCacheKey)));
}
/**
