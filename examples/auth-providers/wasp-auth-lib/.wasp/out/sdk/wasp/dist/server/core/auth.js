 ...
 */
export function requireSessionProvider(requiredProviderIds) {
    return defineHandler(async (req, _res, next) => {
        if (req.user == null) {
            throw createInvalidCredentialsError();
        }
        if (!requiredProviderIds.includes(req.user.sessionProviderId)) {
            throw new HttpError(403, `Authenticated via '${req.user.sessionProviderId}', but this requires signing in via one of: ${requiredProviderIds.join(', ')}.`);
        }
        next();
    });
}
