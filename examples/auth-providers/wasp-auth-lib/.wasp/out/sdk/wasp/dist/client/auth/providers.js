import { api, getLastAuthProviderId, getSessionId, removeLocalUserData, } from '../../api/index.js';
...
function makeClientRuntime(providerId, declaredClientEnvVarNames) {
    return {
        apiUrl: config.apiUrl,
        env: Object.fromEntries(declaredClientEnvVarNames.map((name) => [
            name,
            env[name],
        ])),
...
// PRIVATE API
export const clientAuthAdapters = {
    'external:wasp-auth': createClientAdapter_0(makeClientRuntime('external:wasp-auth', []), { "methods": { "usernameAndPassword": {}, "email": { "emailVerificationPath": "/email-verified", "passwordResetPath": "/password-reset" }, "google": {} }, "oauthCallbackPath": "/oauth/callback" }),
};
// PUBLIC API
/**
...
async function attemptResumeSession() {
    ...
    const adapter = clientAuthAdapters[lastProviderId];
    ...
    try {
        await exchangeCredentialForSession(lastProviderId, credential);
    }
    catch {
        return false;
    }
 ...
 */
export async function loginWithAuthProvider(providerId) {
    const adapter = clientAuthAdapters[providerId];
    if (adapter?.getCredential === undefined) {
        throw new Error(`Auth provider '${providerId}' has no client-side credential source; ` +
            `obtain the credential yourself and call exchangeCredentialForSession().`);
    }
    const credential = await adapter.getCredential();
    if (credential === null) {
        throw new Error(`Auth provider '${providerId}' has no active credential; ` +
...
for (const [providerId, adapter] of Object.entries(clientAuthAdapters)) {
    wireAdapterCredentialEvents(providerId, adapter);
}
function wireAdapterCredentialEvents(providerId, adapter) {
    ...
    let lastKnownCredential = undefined;
