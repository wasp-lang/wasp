...
// To run this script, position yourself in the parent dir of your wasp project, and run:
//   node migrate-0-11.js <your-wasp-project-dir-name>
// Make sure that you have wasp version 0.12 installed globally as `wasp-cli`.

const WASP_BIN = 'wasp-cli';  // TODO: Change this to be 0.12 once we release the script, this is now `wasp-cli` just for us for testing during development.

const projectDirName = process.argv[2].replace(/\/$/, '');
if (projectDirName.split(path.sep).length > 1) {
  console.error('Please provide the name of the project, not the path.');
  process.exit(1);
}
const oldProjectDirName = projectDirName + '_old';
...
// 13
console.log('13. Running wasp clean...');
