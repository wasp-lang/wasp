...
- `wasp start` and `wasp build start` now intelligently choose ports so that you can run multiple apps in your system simultaneously, and print the URLs your app is running on. You can also choose the ports yourself with the `--client-port <port>` and `--server-port <port>` arguments. ([#4585](https://github.com/wasp-lang/wasp/pull/4585))
