...
- You can now import `WaspSpecUserError` from `@wasp.sh/spec` and throw it from your `*.wasp.ts` files or spec helper libraries to report configuration problems as clean, actionable errors, the same way Wasp's spec pipeline does. ([#4448](https://github.com/wasp-lang/wasp/pull/4448))
