...
- Added the `server.basePath` option, which mounts the whole server under a path of your choice: Wasp's own routes (`/auth/*`, `/operations/*`, `/crud/*`, the WebSocket path), your `api`/`apiNamespace` routes (declared relative to it) and routes added in `setupFn`. It defaults to `/`, and new projects set it to `/api`. If you set it, an `api` declared at `/foo/bar` is served at `<basePath>/foo/bar`, OAuth redirect URIs become `<server origin><basePath>/auth/<provider>/callback` (e.g. `http://localhost:3001/api/auth/google/callback`), the client's `config.apiUrl` includes the base path (so `api.get('/foo/bar')` from `wasp/client/api` hits `<basePath>/foo/bar`), and server code can read it as `config.serverBasePath` from `wasp/server`. `WASP_SERVER_URL` and `REACT_APP_API_URL` stay the server's origin. ([#4784](https://github.com/wasp-lang/wasp/issues/4784))
- Added the `ignoreServerBasePath` option to `api` and `apiNamespace`, which serves the endpoint at its path on the server's origin root instead of under `server.basePath`, for paths dictated from the outside like `/.well-known/atproto-did`. Such endpoints are matched after the routes under the base path. An `apiNamespace` applies only to `api`s on the same side of the base path as itself. ([#4784](https://github.com/wasp-lang/wasp/issues/4784))
...
- Wasp no longer provides Tailwind CSS dependencies automatically. You must install them yourself. ([#3571](https://github.com/wasp-lang/wasp/pull/3571))
