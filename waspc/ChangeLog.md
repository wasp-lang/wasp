...
- Wasp now requires the `wasp()` plugin in your `vite.config.ts` file. This plugin must be imported from `wasp/client/vite` and added as the first plugin in your Vite configuration. ([#3652](https://github.com/wasp-lang/wasp/pull/3652))
- The `@source(".")` directive is no longer needed in Tailwind CSS imports. The `wasp()` plugin now handles content scanning automatically. ([#3652](https://github.com/wasp-lang/wasp/pull/3652))
...
- **User-land Vite configuration**: You now have full control over your `vite.config.ts` file, allowing you to customize your frontend build process without limitations. ([#3652](https://github.com/wasp-lang/wasp/pull/3652))
- The `wasp()` plugin now accepts `WaspPluginOptions` to customize the underlying `@vitejs/plugin-react` behavior. ([#3652](https://github.com/wasp-lang/wasp/pull/3652))
