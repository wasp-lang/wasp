...
### Express `app` and `server` available in server `setupFn`
- Wasp now passes in a context to the server `setupFn` that contains Express `app` and `server` objects. This can be used as an escape hatch for things like custom routes or WebSocket support.
