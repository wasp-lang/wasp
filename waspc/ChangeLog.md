...
- Newly created projects no longer open the browser automatically on `wasp start`. To get the old behaviour back, set `server.open` to `true` in your `vite.config.ts`.
