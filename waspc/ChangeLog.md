...
- Fixed logging out removing your app's own `localStorage` keys whose names start with `wasp` (e.g. `waspello-theme`). Wasp now only removes its own `wasp:`-prefixed keys.
