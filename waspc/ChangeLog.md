...
- Wasp now manages your app's URLs in development, so setting `WASP_SERVER_URL`, `WASP_WEB_CLIENT_URL`, or `REACT_APP_API_URL` yourself is now an error. Use `--client-url` and `--server-url` instead. ([#4692](https://github.com/wasp-lang/wasp/pull/4692), [#XXXX](https://github.com/wasp-lang/wasp/pull/XXXX))
- Wasp now manages your app's ports in development, so setting them yourself (e.g. `server.port` in `vite.config.ts`) is now an error. Use `--client-port` and `--server-port` instead. ([#4591](https://github.com/wasp-lang/wasp/pull/4591))
...
- `wasp start` and `wasp build start` now accept `--client-url <url>` and `--server-url <url>` arguments to set your app's public URLs in development, for example when testing on your local network or exposing the server through an HTTPS tunnel. The port arguments keep controlling the local ports your app listens on. ([#XXXX](https://github.com/wasp-lang/wasp/pull/XXXX))
