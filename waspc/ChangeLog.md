...
- Auth route references in the Wasp Spec are now type-checked: `onAuthFailedRedirectTo`, `onAuthSucceededRedirectTo`, and the email `emailVerification`/`passwordReset` `clientRoute` fields now take a `route()` object. ([#4406](https://github.com/wasp-lang/wasp/pull/4406))
