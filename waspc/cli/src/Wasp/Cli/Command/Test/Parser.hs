...
import Wasp.Cli.Parser.Util (mkCommandWithInfoModifiers)

testParser :: Parser CommandCall
testParser = Test <$> testRestArgsParser
  where
    testRestArgsParser = O.subparser $ mkCommandWithInfoModifiers "client" [O.progDesc "Run your app client tests.", O.forwardOptions] (TestClient <$> O.many testRestArgs)
