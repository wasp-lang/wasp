...
import Wasp.Cli.Command.Call (CompletionArgs (..), Shell (..))
  ...
  liftIO . putStrLn $
    unlines
      [ "Run the following command to generate shell (bash, zsh, or fish) completion script for wasp on your machine:",
        ...
        "",
        "After that, you will need to edit your bash profile, by editing `~/.bash_profile` (usually on Linux)",
        "or `~/.bashrc` (usually on MacOS) file and adding this line:",
