    ...
    describe "auth-related validation" $ do
      ...
      describe "should validate that UsernameAndPassword and Email auth cannot used at the same time" $ do
        ...
        it "returns no error if app.auth is not set" $ do
          ASV.validateAppSpec (makeSpec (AS.Auth.AuthMethods {usernameAndPassword = Nothing, slack = Nothing, discord = Nothing, google = Nothing, keycloak = Nothing, gitHub = Nothing, linkedin = Nothing, microsoft = Nothing, email = Nothing}) validUserEntity) `shouldBe` []
        ...
        it "returns no error if app.auth is set and only one of UsernameAndPassword and Email is used" $ do
          ASV.validateAppSpec
            ...
            `shouldBe` []
          ASV.validateAppSpec (makeSpec (AS.Auth.AuthMethods {usernameAndPassword = Nothing, slack = Nothing, discord = Nothing, google = Nothing, keycloak = Nothing, gitHub = Nothing, linkedin = Nothing, microsoft = Nothing, email = Just emailAuthConfig}) validUserEntity) `shouldBe` []
                        ...
                        basicApp
                          { AS.App.auth =
                              Just
                                AS.Auth.Auth
                                  { AS.Auth.methods =
                                      AS.Auth.AuthMethods {email = Just emailAuthConfig, usernameAndPassword = Nothing, slack = Nothing, discord = Nothing, google = Nothing, keycloak = Nothing, gitHub = Nothing, linkedin = Nothing, microsoft = Nothing},
                                    AS.Auth.userEntity = AS.Core.Ref.Ref userEntityName,
                                    AS.Auth.onAuthFailedRedirectTo = "/",
                                    AS.Auth.onAuthSucceededRedirectTo = Nothing,
                                    AS.Auth.onBeforeSignup = Nothing,
