...
\begin{enumerate}
  ...
  \item Type checking of statements:
        \begin{enumerate}
          \item \textsc{Call}, \textsc{Defn}, and the premise of \textsc{Decl}
...
evaluated in order. The evaluation order of declarations and calls is left
unspecified. When declarations and calls are evaluated, so are their arguments.

Only the values of declarations and calls are visible to the caller of the wasp
interpreter.
...
func auth = { userEntity : entity
            , methods : [authMethod]
            , onAuthFailedRedirectTo : route
            }
func dependencies = json
\end{verbatim}
...
\begin{itemize}
  \item \texttt{route} is now a declaration instead of a function
...
newtype Dependencies = Dependencies WaspLang.Primitive.Json
                       deriving (Generic)

interpretWithLib :: String -> Either WaspLang.Error WaspLang.Result
interpretWithLib = WaspLang.interpret $ do
  WaspLang.registerEnumType @ParamType
  WaspLang.registerDataType @Page
  WaspLang.registerFuncType @Dependencies

-- To use pages declared by a wasp file
printPageComponents :: WaspLang.Result -> IO ()
printPageComponents result = do
  let pageDecls = WaspLang.Decl.take @Page $ WaspLang.Result.decls result
  mapM_
...
\texttt{registerEnumType}, \texttt{registerDataType}, and \texttt{registerFuncType}
would all be implemented using GHC generics. Additionally, some type classes
would be used to only allow types in specific forms:
...
registerFuncType :: (Generic a, IsWaspType a) => WaspMonad ()
\end{lstlisting}
\end{minipage}
...
The type for \texttt{Call} (complementary to \texttt{registerFuncType}) would
look very similar.
...
should not be allowed.Evaluating declarations and calls is a bit more nuanced,
since they can refer to declarations declared lexically after themselves.

One possible solution is sort the declarations and calls topographically before
...
evaluation available to every single declaration and call, which solves
...
in \texttt{Result.decls} and \texttt{Result.calls}. I have investigated the
\texttt{Data.Data} module and I am confident this will work, but
\texttt{unsafeCoerce} might be needed.
