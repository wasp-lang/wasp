...
Server is written in NodeJS and uses ExpressJs, bridged into [Nitro](https://nitro.build/).
Database is abstracted via Prisma.

Client and server are not two programs: one Nitro server serves the app's pages
and its API alike, built from a single Vite config. In development that is a
single Vite process; `wasp build` produces a single deployable output from it.

We can run Wasp project with `wasp start`.
This will first compile the app, generate JS code in the `.wasp/out/` dir, and then run `npx vite` for the app, and also run the database.
On any changes you do to the source code of Wasp, Wasp project gets recompiled, and then changes in the generated app are picked up by Vite, therefore updating the web app.
