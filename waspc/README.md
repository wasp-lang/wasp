...
We also wrote a `new-release` script for manually tagging a release: you provide it with a new version (`./new-release 0.3.0`) and it will check that everything is all right, create the appropriate tag and push it. For normal releases the `vX.Y.Z` tag is now created by release-please instead (see [Versioning (release-please)](#versioning-release-please)); `new-release` is mainly used for test / Release Candidate tags (e.g. `./new-release 0.24.1-rc.1`).
...
The Wasp version is bumped by [release-please](https://github.com/googleapis/release-please), configured via `release-please-config.json` and `.release-please-manifest.json` at the repo root.

- Bumps are derived from **conventional commit** messages (`feat:`, `fix:`, `feat!:` / `BREAKING CHANGE:`), so commits need to follow that convention. There is no automated enforcement yet.
- The whole repo bumps in one shared version, driven by the single `waspc` component. release-please watches the `release` branch and opens a **release PR** into `release` that bumps `waspc.cabal`, the `package.json` of each `@wasp.sh/*` lib/package (matched with globbed `extra-files`), and the `wasp: { version }` in each `examples/*/main.wasp.ts`. `wasp-app-runner` is versioned independently (its own commits, its own `wasp-app-runner-v*` tag).
- release-please **cannot** regenerate derived files (every `package-lock.json` and the e2e snapshots), so the release PR's CI stays red until you regenerate them manually (see the release steps below). Each lib/package `package-lock.json` is refreshed from its bumped `package.json` by `./run sync-derived-versions`.
- Merging the release PR creates the `vX.Y.Z` tag. Because release-please creates that tag with `GITHUB_TOKEN`, it does **not** auto-trigger `release-create.yaml`; you trigger that build pipeline manually (see below).
- `ChangeLog.md` is **not** generated by release-please (`skip-changelog`); keep curating it by hand.

The old `./run version-bump` script has been removed; you no longer hand-edit versions. `./run sync-derived-versions` regenerates the derived artifacts from the already-bumped sources.
...
- 👉 The version is bumped automatically by release-please, so you do **not** hand-edit `waspc.cabal` or any other version file. The next version is computed from the conventional commits and shown in release-please's release PR title (see [Versioning (release-please)](#versioning-release-please)).
...
- 👉 Check commits since the latest release, and consider enriching and polishing the `ChangeLog.md` and migration guides. (release-please does not touch `ChangeLog.md`; it stays hand-curated.)
...
- 👉 Switch to the `release` branch and fast-forward it to the RC branch by running `git merge --ff rc-<version>`, then push `release`. This is what release-please watches: it opens (or updates) a **release PR** into `release` that bumps every version file.
- 👉 Regenerate the derived files that release-please can't, on the release PR branch (`release-please--branches--release`). Its CI stays red until you do this:
  - `cd waspc && ./run build` (the bumped `waspc.cabal` and `package.json`s are already on the branch).
  - `./run sync-derived-versions` to rebuild the libs/packages (refreshing their own `package-lock.json`) and refresh each `examples/*/package-lock.json`.
  - `./run test:waspc:e2e:accept-all` to regenerate the e2e snapshots (needs Docker running; `accept-all` can hide mismatches, so also run the comparison-mode e2e test to confirm only version strings changed).
  - Commit `examples/*/package-lock.json` + `waspc/e2e-tests/test-outputs/snapshots/**` and push to the PR branch.
- 👉 Review and merge the release PR. release-please then creates the `vX.Y.Z` tag.
- 👉 Trigger the release build for the new tag. Because release-please creates the tag with `GITHUB_TOKEN`, `release-create.yaml` is **not** auto-triggered: start it manually (re-push the tag, or run the workflow from the Actions UI). When it's done, it will create a new draft release.
