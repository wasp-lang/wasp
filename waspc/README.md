...
Wasp compiler takes `main.wasp.ts`, `schema.prisma`, and everything in the `src/` dir and generates a web app that consists of a client, server, and database.

The compiler has two basic layers: analysis (frontend) and generation (backend).

The analyzer, orchestrated by [`Wasp.Project.Analyze`](src/Wasp/Project/Analyze.hs), parses and validates the Prisma schema, analyzes the TypeScript config through [`Wasp.Project.WaspFile.TypeScript`](src/Wasp/Project/WaspFile/TypeScript.hs), and collects external code and configuration. It constructs and validates the central IR (Intermediate Representation), [`AppSpec`](src/Wasp/AppSpec.hs).
...
The main generators are `ServerGenerator`, `SdkGenerator` (including client and server SDK code), and `DbGenerator`. `DockerGenerator`, `TypeAugmentationGenerator`, and `WaspLibs` provide deployment files, TypeScript type augmentations, and internal libraries.
...
flowchart TB
    ...
    subgraph analyzer["Analyzer"]
        parse["Parse + validate Prisma schema"]
        schema(["Prisma AST"])
        entities["Construct entity declarations"]
        analyze["Analyze TypeScript config"]
        validate["Construct + validate AppSpec"]

        parse --> schema
        schema --> entities --> validate
        schema -->|Model names| analyze
        analyze --> validate
    end
    ...
    config ----> analyze
    source -----> validate
    validate ==> spec(["AppSpec"])

    subgraph generator["Generator"]
        generate["Generate file drafts"]
        server["Server"]
        sdk["Client + server SDK"]
        db["Database"]
        other["Deployment files,<br/>types + libraries"]
        serverDrafts["FileDrafts"]
        sdkDrafts["FileDrafts"]
        dbDrafts["FileDrafts"]
        otherDrafts["FileDrafts"]

        generate --> server & sdk & db & other
        server --> serverDrafts
        sdk --> sdkDrafts
        db --> dbDrafts
        other --> otherDrafts
    ...
    spec ==> generate
    templates["Templates + bundled libraries"] -.-> generate
    drafts(["Combined FileDrafts"])
    serverDrafts --> drafts
    sdkDrafts --> drafts
    dbDrafts --> drafts
    otherDrafts --> drafts
    drafts ==> write["Write to disk"] ==> app["Full-stack app"]

    classDef phase fill:#f6f6f1,stroke:#333,stroke-width:2px
    classDef input fill:#fff0b3,stroke:#333,color:#111
    classDef representation fill:#ccdef5,stroke:#333,color:#111
    classDef keyRepresentation fill:#ccdef5,stroke:#333,stroke-width:2px,color:#111,font-weight:bold
    classDef output fill:#b9d8a8,stroke:#333,stroke-width:2px,color:#111,font-weight:bold
    class analyzer,generator phase
    class prisma,config,source,templates input
    class schema,serverDrafts,sdkDrafts,dbDrafts,otherDrafts,drafts representation
    class spec keyRepresentation
    class app output
```

Generated web app consists of a client, server and database.

Client is written with React and TanStack Query (`@tanstack/react-query`).
...
- `data/Generator/libs/` -> internal npm packages (WaspLibs) that are bundled with Wasp and copied into generated apps (see [WaspLibs](#wasplibs) for more details)
- `data/packages/` -> TypeScript packages used by Wasp compiler (see [TypeScript Packages](#typescript-packages) for more details)
...
WaspLibs are internal npm packages located under [`data/Generator/libs/`](data/Generator/libs/) and are bundled with Wasp and installed into generated apps. A more detailed description of WaspLibs can be found in the [`data/Generator/libs/README.md`](data/Generator/libs/README.md).
