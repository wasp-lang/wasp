...
module Wasp.Analyzer.StdTypeDefinitions.App.ModuleEntityMap () where
...
-- | ModuleEntityMap is never constructed from the Wasp DSL (.wasp files).
...
instance HasCustomEvaluation ModuleEntityMap where
      ...
      ER.mkEvaluationError ctx $
        ER.ParseError $
          ER.EvaluationParseError
            "ModuleEntityMap cannot be constructed from the Wasp DSL"
