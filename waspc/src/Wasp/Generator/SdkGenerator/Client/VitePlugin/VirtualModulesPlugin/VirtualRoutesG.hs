...
import Wasp.JsImport (getJsImportPathString)
...
createRouteTemplateData spec (name, route) =
  ...
  where
    ...
    jsImport = GJI.extImportToRelativeSrcImportFromViteExecution pageComponent
    importPath = getJsImportPathString jsImport
    isAuthRequired = fromMaybe False $ AS.Page.authRequired $ snd targetPage
    isDefault = case EI.name pageComponent of
      EI.ExtImportModule _ -> True
      EI.ExtImportField _ -> False
    exportedName = EI.importIdentifier pageComponent
