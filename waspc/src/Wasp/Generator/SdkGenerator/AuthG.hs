...
genUseAuth auth = return $ mkTmplFdWithData tmplFile tmplData
  where
    tmplFile = authDirInSdkTemplatesDir </> [relfile|useAuth.ts|]
...
genLuciaTs auth = return $ mkTmplFdWithData tmplFile tmplData
  where
    tmplFile = authDirInSdkTemplatesDir </> [relfile|lucia.ts|]
    tmplData =
      object
...
genSessionTs auth = return $ mkTmplFdWithData tmplFile tmplData
  where
    tmplFile = authDirInSdkTemplatesDir </> [relfile|session.ts|]
    tmplData =
      object
...
genUtils auth = return $ mkTmplFdWithData tmplFile tmplData
  where
    tmplFile = authDirInSdkTemplatesDir </> [relfile|utils.ts|]
    tmplData =
      object
        [ "userEntityUpper" .= (userEntityName :: String),
...
genIndexTs auth = return $ mkTmplFdWithData tmplFile tmplData
  where
    tmplFile = authDirInSdkTemplatesDir </> [relfile|index.ts|]
    tmplData =
      object
...
genProvdersIndex auth = return $ mkTmplFdWithData tmplFile tmplData
  where
    tmplFile = authDirInSdkTemplatesDir </> [relfile|providers/index.ts|]
    tmplData =
      object
...
genProvidersTypes auth = return $ mkTmplFdWithData tmplFile tmplData
  where
    tmplFile = authDirInSdkTemplatesDir </> [relfile|providers/types.ts|]
    tmplData =
      object
        [ "userEntityUpper" .= (userEntityName :: String),
          "emailUserSignupFields" .= extImportToImportJson userEmailSignupFields,
