...
import Wasp.Generator.SdkGenerator.Core.Common (SdkTemplatesCoreProjectDir)
import qualified Wasp.SemanticVersion as SV

data EmailSenderProvider = EmailSenderProvider
  { npmDependency :: Maybe Npm.Dependency.Dependency,
    setupFnFile :: Path' (Rel SdkTemplatesCoreProjectDir) File'
  }
  deriving (Show, Eq)
...
providersDirInSdkTemplatesDir :: Path' (Rel SdkTemplatesCoreProjectDir) Dir'
