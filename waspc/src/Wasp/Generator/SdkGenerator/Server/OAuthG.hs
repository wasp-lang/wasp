...
import Wasp.Generator.AuthProviders (discordAuthProvider, getEnabledAuthProvidersJson, gitHubAuthProvider, googleAuthProvider, keycloakAuthProvider, linkedinAuthProvider, microsoftAuthProvider, slackAuthProvider)
...
genOAuth auth
  | AS.Auth.isExternalAuthEnabled auth =
      sequence
        ...
        <++> genOAuthProvider linkedinAuthProvider (AS.Auth.linkedin . AS.Auth.methods $ auth)
...
depsRequiredByOAuth spec =
  [Npm.Dependency.make ("arctic", "^1.8.0") | (AS.App.Auth.isExternalAuthEnabled <$> maybeAuth) == Just True]
  where
