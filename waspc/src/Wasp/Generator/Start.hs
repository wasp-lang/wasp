...
import Wasp.Generator.ServerGenerator.Start (ServerProcessCommandQueue, startServer)
...
start :: Path' Abs (Dir WaspProjectDir) -> Path' Abs (Dir GeneratedAppDir) -> ServerProcessCommandQueue -> IO () -> IO (Either String ())
start waspProjectDir outDir serverProcessCommandQueue onJobsQuietDown = do
  chan <- newChan
  let runStartJobs = startServer outDir serverProcessCommandQueue chan `race` startWebApp waspProjectDir chan
