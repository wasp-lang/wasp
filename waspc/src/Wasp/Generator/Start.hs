...
start :: Path' Abs (Dir WaspProjectDir) -> Path' Abs (Dir ProjectRootDir) -> IO () -> IO (Either String ())
start waspProjectDir projectDir onJobsQuietDown = do
  chan <- newChan
  let runStartJobs = startServer projectDir chan `race` startWebApp waspProjectDir projectDir chan
