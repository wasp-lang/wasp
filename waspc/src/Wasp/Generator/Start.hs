...
import Wasp.Generator.ServerGenerator.ServerProcessSupervisor (ServerProcessSupervisor)
...
start :: Path' Abs (Dir WaspProjectDir) -> Path' Abs (Dir GeneratedAppDir) -> ServerProcessSupervisor -> IO () -> IO (Either String ())
start waspProjectDir outDir serverProcessSupervisor onJobsQuietDown = do
  chan <- newChan
  let runStartJobs = startServer outDir serverProcessSupervisor chan `race` startWebApp waspProjectDir chan
