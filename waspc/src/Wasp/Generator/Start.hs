...
import Wasp.Generator.ServerGenerator.Start (ServerProcessManager, startServer)
...
start :: Path' Abs (Dir WaspProjectDir) -> Path' Abs (Dir GeneratedAppDir) -> ServerProcessManager -> IO () -> IO (Either String ())
start waspProjectDir outDir serverProcessManager onJobsQuietDown = do
  chan <- newChan
  let runStartJobs = startServer outDir serverProcessManager chan `race` startWebApp waspProjectDir chan
