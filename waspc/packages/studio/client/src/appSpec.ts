...
export type AppSpec = {
  ...
  isBuild: boolean;
  devEnvVarsServer: string[];
  devEnvVarsClient: string[];
  devDatabaseUrl: string | null;
  userDockerfileContents: string | null;
  customViteConfigPath: string | null;
  srcTsConfigPath: string;
  srcTsConfig: unknown;
  packageJson: unknown;
  tailwindConfigFilesRelocators: unknown;
  prismaSchema: unknown;
  migrationsDir: string | null;
  externalCodeFiles: unknown;
  externalPublicFiles: unknown;
};
...
export type DeclType = Decl["declType"];
...
export type AnyRef = {
  [Type in keyof DeclTypeToValue]: Ref<Type>
}[keyof DeclTypeToValue];
