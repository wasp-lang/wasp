...
export async function createDb(region: string, options: CreateDbOptions): Promise<void> {
	...
	if (!serverTomlExistsInProject(tomlFilePaths)) {
		waspSays(`${tomlFilePaths.serverTomlPath} missing. Skipping DB creation. Perhaps you need to run "${getCommandHelp(flySetupCommand)}" first?`);
		exit(1);
	}

	const inferredBaseName = getInferredBasenameFromServerToml(tomlFilePaths);
	const deploymentInfo = createDeploymentInfo(inferredBaseName, region, options, tomlFilePaths);
