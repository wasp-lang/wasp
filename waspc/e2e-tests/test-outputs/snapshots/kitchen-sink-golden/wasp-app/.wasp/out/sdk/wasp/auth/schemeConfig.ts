// PRIVATE API
/**
 * Rebuilds an auth handler's `config` for its factory.
 *
 ...
 */
export function joinSchemeConfig(
  data: unknown,
  ...
  const config: Record<string, unknown> = isRecord(data) ? structuredClone(data) : {}
