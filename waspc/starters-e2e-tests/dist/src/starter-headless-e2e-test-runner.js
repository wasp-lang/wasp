...
export async function runStarterHeadlessE2ETests(execution) {
...
async function runDevHeadlessE2ETests(templateName, waspProjectPath, waspCliCommand) {
...
async function runBuildHeadlessE2ETests(templateName, tempWaspProjectPath, waspCliCommand) {
    if (await waspProjectUsesSqlite(tempWaspProjectPath, waspCliCommand)) {
        console.log(chalk.yellow(`Skipping BUILD headless e2e tests for ${templateName} starter - uses SQLite`));
        return;
    }
    console.log(`Running BUILD headless e2e tests for ${templateName} starter...`);
