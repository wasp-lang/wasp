...
export function setupTestUser(userSignupFields) {
    const credentials = {
        ...userSignupFields,
...
async function performSignup(page, credentials) {
    await page.goto("/signup");
    await page.waitForSelector("text=Create a new account");
    await submitSignupForm(page, credentials);
    await expect(page.locator("body")).toContainText(`You've signed up successfully! Check your email for the confirmation link.`);
}
async function submitSignupForm(page, credentials) {
...
*/
async function performEmailVerification(page, email) {
...
async function submitLoginForm(page, credentials) {
