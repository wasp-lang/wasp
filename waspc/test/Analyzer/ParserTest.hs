...
spec_Parser = do
  describe "Analyzer.Parser" $ do
    ...
    it "Parses external imports" $ do
      let source = unlines [ "test Imports {"
                           , "  module: import Page from \"page.jsx\","
                           , "  field: import { Page } from \"page.jsx\""
                           , "}"]
      let ast = AST [ Decl "test" "Imports" $ Dict
    ...
    it "Parses quoted PSL" $ do
      let source = unlines [ "test PSL {=psl"
                           , "  id Int @id"
                           , "psl=}"
                           ]
    ...
    it "Parses quoted JSON" $ do
      let source = unlines [ "test JSON {=json"
                           , "  \"key\": \"value\""
                           , "json=}"
                           ]
    ...
    it "Requires dictionaries to have an ending bracket" $ do
      ...
      let expected = Left $ ParseError $ Token { tokenClass = TEOF
                                               , tokenPosn = Posn 1 12
                                               , tokenLexeme = ""
                                               }
    ...
    it "Parses multiple statements" $ do
      let source = unlines [ "constant Pi 3.14159"
                           , "constant E  2.71828"
                           ]
      let ast = AST [ Decl "constant" "Pi" $ DoubleLiteral 3.14159
