 ...
 * `AuthUser`. So we must be able to create a portable reference to
 * `AuthUser`.
 * 
 ...
 *    where `AuthUser` is declared, so this condition always fails.
 * 
 ...
 * in an error, because the `AuthUser` symbol is not portable.
 * 
 * To avoid this problem, `"wasp/server/operations"` re-export `AuthUser`.
