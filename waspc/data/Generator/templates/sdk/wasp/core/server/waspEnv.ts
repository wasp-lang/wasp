...
export const serverDevEnvSchema = z.object({
...
export const serverProdEnvSchema = z.object({
...
const waspServerEnvSchema = z.discriminatedUnion('NODE_ENV', [
  serverDevEnvSchema.merge(commonServerEnvSchema),
  serverProdEnvSchema.merge(commonServerEnvSchema)
])

const defaultNodeEnvValue = serverDevEnvSchema.shape.NODE_ENV.value;
const { NODE_ENV: inputNodeEnvValue, ...restEnv } = process.env;
export const unvalidatedEnv = {
  NODE_ENV: inputNodeEnvValue ?? defaultNodeEnvValue,
  ...restEnv,
}

// PUBLIC API
export const waspEnv = ensureEnvSchema(
  unvalidatedEnv,
  waspServerEnvSchema,
)
