...
export function loadWaspEnvClient(rootDir: string, envPrefix: NonNullable<UserConfig['envPrefix']>): Record<string, string> {
  ...
  const envFilePath = resolve(rootDir, '.env.client')
  ...
  try {
    ...
    console.error('Error parsing .env.client:', error)
    throw error
  }
}
...
export function envFile(): Plugin {
  return {
    ...
    config(config): UserConfig {
      // TODO: figure out how to do this in `configResolved` so we can use `config.envPrefix` instead
      // of the constant.
      const envVars = loadWaspEnvClient(config.root || process.cwd(), ENV_PREFIX)
