...
import type { Action, Query } from "../../operations/rpc.js";
import { makeQueryCacheKey } from "./queries.js";
 ...
 * cache.
 */
type SpecificUpdateQuery<CachedData> = (
  oldData: CachedData | undefined,
) => CachedData;
...
function translateToInternalDefinition<Item, CachedData>(
  publicOptimisticUpdateDefinition: OptimisticUpdateDefinition<
    Item,
    CachedData
  >,
...
function makeRqOptimisticUpdateOptions<ActionInput, CachedData>(
  ...
  UseMutationOptions<
    ...
    unknown,
    ActionInput,
    { previousData: Map<QueryKey, CachedData | undefined> }
  >,
  "onMutate" | "onError"
> {
  async function onMutate(item: ActionInput) {
    const specificOptimisticUpdateDefinitions = optimisticUpdateDefinitions.map(
      (generalDefinition) =>
        getOptimisticUpdateDefinitionForSpecificItem(generalDefinition, item),
    );
