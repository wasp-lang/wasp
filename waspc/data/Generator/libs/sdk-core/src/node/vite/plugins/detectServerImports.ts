...
export function detectServerImports(srcDir: string): Plugin {
  let parsePathToUserCode!: ParsePathToUserCodeFn;
  return {
    name: "wasp:detect-server-imports",
    enforce: "pre",
    configResolved(config) {
      parsePathToUserCode = createPathToUserCodeParser(config.root, srcDir);
    },
...
function createPathToUserCodeParser(
  ...
  return (importerPath: string): RelativePathToUserCode | null => {
    ...
    return importerPathRelativeToWaspProjectDir.startsWith(srcDir)
